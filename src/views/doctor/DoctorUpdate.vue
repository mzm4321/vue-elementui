  ...
  <div>
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item label="医生图片：">
        <el-image :src="ruleForm.doctorImg" class="image"></el-image>
      </el-form-item>
...
<script>
import {getDoctorbyId, insertDoctor, updateDoctor} from "@/api/doctor";
...
export default {
  ...
  created() {
    getDoctorbyId(this.doctorId).then((res)=>{
      this.ruleForm=res.data
      if(this.ruleForm.hospitalTime.length==1){
        this.ruleForm.hospitalTime=[]
      }
    })
  },
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          updateDoctor(this.doctorId,this.ruleForm.doctorName, this.ruleForm.doctorAge, this.ruleForm.history, this.ruleForm.title, this.ruleForm.stop, this.ruleForm.hospitalTime, this.ruleForm.doctorDetail, this.ruleForm.doctorSex, this.ruleForm.doctorImg, this.ruleForm.departId).then((res) => {
            _this.$message.success("修改成功")
            _this.$router.push({path:'/doctor'})
          })
        } else {
